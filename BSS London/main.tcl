...
proc dataParser {data} {
	...
	set new_message [reverseEscapeControlCodes [reverseFinalizeControlString $data]]
	set message_type [dict get [lreverse $::MESSAGE_TYPE] [string range $new_message 0 1]]
	puts $message_type
	set node [string range $new_message 4 9]
	set virtual_device_id [string range $new_message 12 13]
	#set object_id [dict get [lreverse $::ZONES] [string range $new_message 16 25]]
	#puts $object_id
	set object_id [string range $new_message 16 25]
	set zone 0
	set gain_or_source ""
	for {set i 0} {$i < $::NUMBER_OF_ZONES} {incr i} {
	    set zone_number [expr {$i + 1}]
	    set loc_gain_object_id [dict get $::ZONES $zone_number "Gain Object ID"]
	    set loc_source_object_id [dict get $::ZONES $zone_number "Source Object ID"]
	    if {$loc_gain_object_id == $object_id} {
	        set zone $zone_number
	        set gain_or_source "Gain"
	    } elseif {$loc_source_object_id == $object_id} {
	        set zones $zone_number
	        set gain_or_source "Source"
	    }
	}
	puts "Zone: $zone"
	puts "Gain or Source: $gain_or_source"
	set state_variable [string range $new_message 28 33]
	set data_string [string range $new_message 36 49]
	set checksum [string range $new_message 52 53]
...
#puts $ZONES
dataParser "02, 88, 00, 00, 1B, 83, 00, 01, 31, 00, 00, 00, 01, 00, 00, A2, 03"